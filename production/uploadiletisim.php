...
include '../sistem/baglan.php';
 if($_POST){if ($_FILES["resim"]["size"]<1024*1024)
    ...
    $sorgu = $baglan->prepare("INSERT INTO iletisimresimyukleme SET resim=:resim,resimaciklama=:resimaciklama");  
    $sorgu->execute(array(':resim'=> $yeni_ad,':resimaciklama'=>$aciklama)); 
       if ($sorgu){header('Location: iletisim-veri.php?message=0');}
       else{header('Location: iletisim-veri.php?message=1');}}
       else{header('Location: iletisim-veri.php?message=1');}}
       else{header('Location: iletisim-veri.php?message=2');}}
       else{header('Location: iletisim-veri.php?message=3');}}
